...
export const t = (locale: string) => ({
  ...
  partsNotFoundBody:  isFi(locale) ? "Kirjoita tai soita — meillä on verkosto purkaamoita Virossa ja Suomessa ja etsimme sinulle oikean osan. Vastaamme yleensä tunnin sisällä." : isRu(locale) ? "Напишите или позвоните — у нас есть сеть разборок в Эстонии и Финляндии, и мы найдём нужную деталь. Обычно отвечаем в течение часа." : isEn(locale) ? "Write or call — we have a network of dismantlers in Estonia and Finland and will find the right part. We usually reply within one hour." : "Kirjuta või helista — meil on Eesti ja Soome lammutuste võrgustik ning otsime Sulle õige osa. Tavaliselt vastame ühe tunni jooksul tööajal.",
